...
$( document ).ready(function() {
	...
	$("#add_festit_init").click(function(){
		...
		$("#shifts_li").click(function(event){
			clearAll()
			$("#shifts_li").css({"textDecoration":"underline"});
		...
		$("#users_li").click(function(event){
			clearAll();
			$("#users_li").css({"textDecoration":"underline"});
		});
		
		$("#payouts_li").click(function(event){
			clearAll();
			$("#payouts_li").css({"textDecoration":"underline"});
		});
...
function festival_shift_subscribers(){
	clearAll();
	$("#subscription_li").css({"textDecoration":"underline"});
	var coockie = JSON.parse(getCookie("YOUR_CV_INLOG_TOKEN_AND_ID"));
...
function festival_checkbox_listing(){
	clearAll();
	$("#present_li").css({"textDecoration":"underline"});

	var coockie = JSON.parse(getCookie("YOUR_CV_INLOG_TOKEN_AND_ID"));
	api("get_festivals", {"id" : coockie.ID, "hash" : coockie.TOKEN, "select": "active", "festi_id":"invalid"}, festival_checkbox);
}
...
function festival_checkbox(data){
	...
	$("#festivals").change(function(event){
		...
		var coockie = JSON.parse(getCookie("YOUR_CV_INLOG_TOKEN_AND_ID"));
		api("get_shifts",{"id" : coockie.ID, "hash" : coockie.TOKEN}, shift_processing_checkbox);
	...
	for (let x = 0; x < data.length; x++){
		if (data[x].festival_idfestival == festival_idfestival){
			if (first){
				...
				var coockie = JSON.parse(getCookie("YOUR_CV_INLOG_TOKEN_AND_ID"));
				api("get_shift_days",{"id" : coockie.ID, "hash" : coockie.TOKEN}, shift_day_processing_checkbox);
				first = false;
			}
	...
	for (let x = 0; x < data.length; x++){
		if (data[x].idshifts == selected_shift_presense){
			if (first){
				...
				var coockie = JSON.parse(getCookie("YOUR_CV_INLOG_TOKEN_AND_ID"));
				api("get_workdays_subscribers",{"id" : coockie.ID, "hash" : coockie.TOKEN}, get_subscribers_checkbox_callback);
				first = false;
			}
...
function get_subscribers_checkbox_callback(data){
	...
	$(".checkbox_in").change(function(event){
		...
		api("user_present",{"id" : coockie.ID, "hash" : coockie.TOKEN, "user": parseInt(user), "work_day":parseInt(work_day), "in":in__, "out": 2, "present": 2}, function(){
			api("get_workdays_subscribers",{"id" : coockie.ID, "hash" : coockie.TOKEN}, get_subscribers_checkbox_callback);
		})
	...
	$(".checkbox_out").change(function(event){
		...
		api("user_present",{"id" : coockie.ID, "hash" : coockie.TOKEN, "user": parseInt(user), "work_day":parseInt(work_day), "in": 2, "out": out_, "present": 2}, function(){
			api("get_workdays_subscribers",{"id" : coockie.ID, "hash" : coockie.TOKEN}, get_subscribers_checkbox_callback);
		})
	...
	$(".checkbox_present").change(function(event){
		...
		api("user_present",{"id" : coockie.ID, "hash" : coockie.TOKEN, "user": parseInt(user), "work_day":parseInt(work_day), "in": 2, "out": 2, "present": present_}, function(){
			api("get_workdays_subscribers",{"id" : coockie.ID, "hash" : coockie.TOKEN}, get_subscribers_checkbox_callback);
		})
...
function shift_processing_short(data){
	...
	$(".add_user_to_shift").click(function(){
		...
		$("#user_search").keydown(function(){
			...
			var coockie = JSON.parse(getCookie("YOUR_CV_INLOG_TOKEN_AND_ID"));
			api("user_search", {"id" : coockie.ID, "hash" : coockie.TOKEN, "search": user_part}, add_user_search_result);
		});
	});
...
function subscribers_callback(data){
		...
		for(let x=0; x < data.length; x++){
			...
			$(".unsubscribe_user").click(function(event){
				...
				api("user_unsubscribe", {"id" : coockie.ID, "hash" : coockie.TOKEN, "idshifts": shift, "Id_Users": user}, festival_shift_subscribers);
			});
			$(".subscribe_user").off();
			$(".subscribe_user").click(function(event){
				...
				let user = event.target.attributes.id.value;
				let shift = event.target.attributes.shift.value
				if (user == coockie.ID){user = "admin"}
				api("user_subscribe", {"id" : coockie.ID, "hash" : coockie.TOKEN, "idshifts": shift, "Id_Users": user}, festival_shift_subscribers);
			});
...
function clearAll(){
	...
	$("#festivals_li").css({"textDecoration":"none"});
	$("#shifts_li").css({"textDecoration":"none"});
	$("#subscription_li").css({"textDecoration":"none"});
	$("#present_li").css({"textDecoration":"none"});
	$("#users_li").css({"textDecoration":"none"});
	$("#payouts_li").css({"textDecoration":"none"});
